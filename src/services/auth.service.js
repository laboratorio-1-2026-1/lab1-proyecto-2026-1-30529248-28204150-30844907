...
class AuthService {
  ...
  async getAllUsers(page = 1, limit = 10, includeInactive = false) {
    ...
    const where = {};
    if (!includeInactive) where.estado = ESTADOS_GENERALES.ACTIVO;
  ...
  async updateUser(userId, updateData, requesterId) {
    ...
    if (rolNombre) {
      ...
      // No permitir que un usuario modifique su propio rol
      if (parseInt(requesterId) === parseInt(userId)) {
        throw { status: 403, code: CODIGOS_ERROR.PERMISO_DENEGADO, message: 'No puedes modificar tu propio rol' };
      }
